...
Ordinary kills fill one shared super meter, even before a weapon is equipped. A full meter can activate any selected weapon that is not already active, then resets for all weapons. Super-powered kills cannot refill it. Initial requirements are 17 kills, or 30 in Swarm. Later quotas adapt to recent kill rate to target about 25 seconds of charging, without a hard timer. The combat display labels the shared charge and shows the selected weapon; the bar glows when ready. Q / E changes the weapon to fire without losing charge, and Space spends the full meter. Replays use `containment-2.7.0` for shared charging and seeded encounters; older balance versions are rejected.
